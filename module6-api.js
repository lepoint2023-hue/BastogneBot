...
async function callGemini(userMessage) {
  ...
  try {
    ...
    /* ── Effet typewriter — affichage progressif mot par mot ── */
    _callAddMsg("bot", "▍");
    ...
    if (bubbleEl && reply) {
      ...
      let current  = '';
      const SPEED  = 28; /* ms par mot */
      for (let i = 0; i < words.length; i++) {
        current += (i === 0 ? '' : ' ') + words[i];
        const display = current + (i < words.length - 1 ? ' ▍' : '');
        if (typeof window.parseMarkdown === 'function') {
          bubbleEl.innerHTML = window.parseMarkdown(display);
        } else {
      ...
      /* Rendu final propre */
      if (typeof window.parseMarkdown === 'function') {
        bubbleEl.innerHTML = window.parseMarkdown(reply);
      } else {
