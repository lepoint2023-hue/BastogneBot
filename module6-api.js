...
async function callGemini(userMessage) {
  ...
  try {
    ...
    if (!response.ok) {
      hideTyping();
      console.error('[OdeBot] HTTP Error:', response.status);
      response.text().then(t => console.error('[OdeBot] Body:', t.slice(0,200))).catch(()=>{});
    ...
    console.log('[OdeBot] Content-Type:', contentType);
